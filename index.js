...
// Versión: 7.4.0 - Syntax Fix (Backticks restaurados)
// =============================================================================
...
const Step1PersonalInfo = ({ formData, setFormData, departments, teachers, allCourses, setSelectedCourses, setOriginalSelectedCourses, onNext }) => {
    ...
    return React.createElement('div', { className: 'max-w-4xl mx-auto' },
        ...
        React.createElement('div', { className: 'bg-white p-6 sm:p-8 rounded-2xl shadow-xl border-t-8 border-blue-900' },
            React.createElement('div', { className: 'mb-6 pb-4 border-b border-gray-100' },
                React.createElement('h2', { className: 'text-2xl font-bold text-gray-800' }, 'Datos Personales'),
                React.createElement('p', { className: 'text-sm text-gray-500' }, 'Ingrese sus datos para iniciar.')
            ),
                ...
                React.createElement('div', null,
                    ...
                    React.createElement('input', { 
                        ...
                        onChange: e => {
                            ...
                            setFormData(prev => {
                                const nextData = {...prev, curp: val};
                                if (val.length === 18) {
                                    const gen = getGenderFromCurp(val);
                                    if (gen) nextData.gender = gen;
                                }
                                return nextData;
                            });
                        }
...
const Step2CourseSelection = ({ courses, selectedCourses, setSelectedCourses, originalSelectedCourses, onNext, onBack }) => {
    ...
    return React.createElement('div', { className: 'bg-white p-6 sm:p-8 rounded-2xl shadow-xl border-t-8 border-blue-900' },
        React.createElement('div', { className: 'flex justify-between items-center mb-6 pb-4 border-b border-gray-100' },
            React.createElement('h2', { className: 'text-xl sm:text-2xl font-bold text-gray-800' }, 'Selección de Cursos'),
            React.createElement('div', { className: 'bg-blue-50 text-blue-800 px-3 py-1 sm:px-4 sm:py-2 rounded-full font-bold shadow-sm text-sm sm:text-base' }, 
                `${selectedCourses.length} / 3`
            )
        ),
        
        React.createElement('div', { className: 'max-h-[550px] overflow-y-auto custom-scrollbar p-2 space-y-6' },
            Object.entries(groupedCourses).map(([dateLabel, groupCourses]) => 
                React.createElement('div', { key: dateLabel },
                    React.createElement('div', { className: 'flex items-center gap-3 mb-3' },
                        React.createElement('div', { className: 'h-6 w-1 bg-blue-500 rounded-full' }),
                        React.createElement('h3', { className: 'text-base sm:text-lg font-bold text-slate-700' }, dateLabel)
                    ),
                    React.createElement('div', { className: 'grid gap-3 sm:gap-4 md:grid-cols-2 lg:grid-cols-3' },
                        groupCourses.map(c => {
                            ...
                            return React.createElement('div', { 
                                ...
                                React.createElement('h4', { className: 'font-bold text-slate-800 mb-2 pr-6 leading-tight text-sm sm:text-base' }, c.name),
        ...
        React.createElement('div', { className: 'mt-6 flex justify-between pt-4 border-t border-gray-100' },
...
const Step3Confirmation = ({ formData, courses, originalCourses, onSubmit, onBack }) => {
    ...
    return React.createElement('div', { className: 'bg-white p-8 rounded-2xl shadow-xl border-t-8 border-blue-900 max-w-3xl mx-auto' },
        ...
        toAdd.length > 0 && React.createElement('div', { className: 'mb-4' },
            React.createElement('h4', { className: 'font-bold text-green-700 mb-2 flex items-center gap-2' }, '✨ Altas (Nuevos Cursos)'),
            React.createElement('div', { className: 'bg-green-50 rounded-xl border border-green-100 overflow-hidden' },
                toAdd.map(c => React.createElement('div', { key: c.id, className: 'p-3 border-b border-green-100 last:border-0' }, 
                    React.createElement('p', { className: 'font-bold text-green-900 text-sm' }, c.name),
                    React.createElement('p', { className: 'text-xs text-green-700 mt-0.5' }, `${c.dates}`)
                ))
            )
        ),
        
        toDrop.length > 0 && React.createElement('div', { className: 'mb-4' },
            React.createElement('h4', { className: 'font-bold text-red-700 mb-2 flex items-center gap-2' }, '🗑️ Bajas (Cursos a Cancelar)'),
            React.createElement('div', { className: 'bg-red-50 rounded-xl border border-red-100 overflow-hidden' },
                toDrop.map(c => React.createElement('div', { key: c.id, className: 'p-3 border-b border-red-100 last:border-0' }, 
                    React.createElement('p', { className: 'font-bold text-red-900 text-sm' }, c.name),
                    React.createElement('p', { className: 'text-xs text-red-700 mt-0.5' }, 'Se liberará este lugar')
                ))
            )
        ),
...
const Step4Success = ({ registrationResult, applicantName, emailSent }) => React.createElement('div', { className: 'text-center p-8 sm:p-12 bg-white rounded-2xl shadow-2xl max-w-3xl mx-auto border-t-8 border-green-500' },
    React.createElement('div', { className: 'text-7xl sm:text-8xl mb-6' }, '🎉'),
    ...
    registrationResult && registrationResult.length > 0 && React.createElement('div', { className: 'bg-slate-50 p-6 rounded-xl mb-8 text-left border border-slate-100' },
        ...
        registrationResult.map((r, i) => React.createElement('div', { key: i, className: 'flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 pb-4 border-b border-slate-200 last:border-0 last:pb-0 last:mb-0' },
            React.createElement('div', { className: 'mb-2 sm:mb-0 mr-2' },
                React.createElement('p', { className: 'font-bold text-slate-800 text-sm' }, r.name || 'Curso Registrado'),
                React.createElement('span', { className: 'text-xs text-green-600 font-bold bg-green-100 px-2 py-1 rounded inline-block mt-1' }, r.status)
            ),
            React.createElement('span', { className: 'font-mono font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded border border-blue-100 text-sm whitespace-nowrap' }, r.folio)
        ))
    ),
...
const Stepper = ({ currentStep, steps }) => React.createElement('div', { className: 'flex justify-between items-center w-full max-w-3xl mx-auto my-6 px-4' }, steps.map((s, i) => React.createElement('div', { key: i, className: `flex items-center ${i < steps.length - 1 ? 'flex-1' : ''}` },
    React.createElement('div', { className: 'relative flex flex-col items-center z-10' },
        React.createElement('div', { className: `flex items-center justify-center w-10 h-10 rounded-full font-bold text-base transition-all duration-500 border-4 ${i + 1 <= currentStep ? 'bg-blue-900 text-white border-blue-900 shadow-lg scale-110' : 'bg-white text-slate-300 border-slate-200'}` }, i + 1),
        React.createElement('span', { className: `absolute top-12 text-[10px] sm:text-xs font-bold whitespace-nowrap tracking-wide ${i + 1 <= currentStep ? 'text-blue-900' : 'text-slate-300'}` }, s)
    ),
    i < steps.length - 1 && React.createElement('div', { className: `flex-1 h-1 mx-2 rounded-full transition-all duration-700 ${i + 1 < currentStep ? 'bg-blue-900' : 'bg-slate-200'}` })
)));
...
const App = () => {
    ...
    if (loading && step === 1) return React.createElement('div', { className: 'min-h-[400px] flex flex-col items-center justify-center bg-slate-50' }, 
        React.createElement('div', { className: 'animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-blue-900 mb-4' }),
        React.createElement('p', { className: 'text-slate-600 font-bold text-lg' }, 'Cargando sistema...')
    );

    // LAYOUT MODIFICADO: SIN MIN-H-SCREEN Y BOTÓN REUBICADO
    return React.createElement('div', { className: 'bg-slate-50 py-4 font-sans flex flex-col justify-center min-h-[600px]' },
        
        React.createElement('div', { className: 'container mx-auto px-4 max-w-4xl' },
            
            // BOTÓN VOLVER REUBICADO
            React.createElement('div', { className: 'mb-4 flex justify-start' },
                React.createElement('a', { href: 'index.html', className: 'inline-flex items-center text-blue-700 hover:text-blue-900 font-bold transition-colors text-sm bg-white px-4 py-2 rounded-full shadow-sm border border-blue-100 hover:shadow-md' }, '← Volver al Portal')
            ),

            error && React.createElement('div', { className: 'bg-red-50 text-red-700 p-4 mb-6 rounded-xl border-l-4 border-red-500 shadow-sm flex items-center gap-4' }, 
